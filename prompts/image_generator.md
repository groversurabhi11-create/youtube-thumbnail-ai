...
4. Enforce the no-text constraint at the render level. Text leaking into
   generated images despite explicit "no text" instructions is a known
   failure mode of image models — check for it, and regenerate with stronger
   negative-prompt emphasis if it appears, rather than passing it through.
...
This worked example deliberately shows one clean render and one render that
...
deviations" every time — text leaking into gauge/dial-style imagery
(tick marks, numerals) is a known real failure mode worth calling out
